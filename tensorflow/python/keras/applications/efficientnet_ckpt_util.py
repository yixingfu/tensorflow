"""Util for EfficientNet models for Keras.
Write weights from ckpt file as in original repo
(https://github.com/tensorflow/tpu/tree/master/models/official/efficientnet)
to h5 file for keras.applications implementation of the models.
"""
...
def write_ckpt_to_h5(path_h5, path_ckpt, model_builder, use_ema=True):
  ...
  Args:
    ...
    path_ckpt: str, path to the ckpt files (e.g. 'efficientnet-b0/model.ckpt')
      ...
      https://github.com/tensorflow/tpu/tree/master/models/official/efficientnet
    model_builder: method, the keras.applications efficientnet model builder 
      function (e.g. EfficientNetB0)
    use_ema: Bool, whether to use ExponentialMovingAverage result or not
  """
  model_name_keras, model_name_tf = get_model_name(model_builder)
  ...
  with tf.compat.v1.Session() as sess:
    ...
    for keras_block, tf_block in zip(blocks_keras, blocks_tf):
      print(f'writing {tf_block} to {keras_block}')
      for keras_name in keras_weights:
        if keras_block in keras_name:
          ...
          for v in v_all:
            if v.name == tf_name:
              v_val = sess.run(v)
              with h5py.File(f'{model_name_keras}.h5', 'a') as f:
                v_prev = f[keras_name][()]
    ...
    for keras_name in keras_weights:
      if any([x in keras_name for x in ['stem', 'top', 'probs']]):
...
def get_model_name(model_builder):
  ...
  model_name_keras = model_builder._keras_api_names[0].split('.')[-1].lower()
  model_name_tf = model_name_keras.replace('efficientnet', 'efficientnet-')
  return model_name_keras, model_name_tf
